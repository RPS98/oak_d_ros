...
void OakDTaskDetections::start(ros::NodeHandle& nh){
    ...
    rgb_video_pub = nh.advertise<sensor_msgs::Image>("/rgb_video", 1);
...
void OakDTaskDetections::run(std::vector<std::shared_ptr<dai::DataOutputQueue>>& streams_queue, 
    ...
    rgb_video = streams_queue[queue_index.inx_video]->get<dai::ImgFrame>();
    ...
    cv::Mat frame_video = OakDUtils::getCvFrame(rgb_video);
    ...
    cv_bridge::CvImage frame_video_cv_bridge = cv_bridge::CvImage(header,sensor_msgs::image_encodings::BGR8, frame_video);
    ...
    frame_video_cv_bridge.toImageMsg(frame_video_msg);
    ...
    rgb_video_pub.publish(frame_video_msg);
