...
void OakDTaskStereoNeuralInference::start(ros::NodeHandle& nh){
    ...
    image_right_pub = nh.advertise<sensor_msgs::Image>("/image_rectified_right", 1);
    image_left_pub = nh.advertise<sensor_msgs::Image>("/image_rectified_left", 1);
...
void OakDTaskStereoNeuralInference::run(std::vector<std::shared_ptr<dai::DataOutputQueue>>& streams_queue, 
    ...
    cv::Mat frame_rectifiedRight_pub = OakDUtils::getCvFrame(rectified_right);
    ...
    cv::Mat frame_rectifiedLeft_pub = OakDUtils::getCvFrame(rectified_left);
    ...
    cv_bridge::CvImage frame_cv_bridge_right_full = cv_bridge::CvImage(header,sensor_msgs::image_encodings::MONO8, frame_rectifiedRight_pub);
    cv_bridge::CvImage frame_cv_bridge_left_full = cv_bridge::CvImage(header,sensor_msgs::image_encodings::MONO8, frame_rectifiedLeft_pub);
    ...
    frame_cv_bridge_left_full.toImageMsg(image_left);
    frame_cv_bridge_left_full.toImageMsg(image_right);
...
void OakDTaskStereoNeuralInference::stop(){
    ...
    image_left_pub.shutdown();
    image_right_pub.shutdown();
};