...
void OakDTaskRGBDetections::run(std::vector<std::shared_ptr<dai::DataOutputQueue>>& streams_queue, 
    ...
    for(const auto& d : dets) {
        ...
        bbox.xmax = (int)x2;
        bbox.ymax = (int)y2;
        bbox.depth = (float)d.spatialCoordinates.z/1000;      // m
        bbox.x_centroid = (float)d.spatialCoordinates.x/1000; // m
        bbox.y_centroid = (float)d.spatialCoordinates.y/1000; // m
